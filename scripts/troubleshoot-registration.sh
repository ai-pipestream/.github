...
probe_service() {
    ...
    # Effective config (key properties)
    local config_out
    config_out=$(curl -sf --max-time 3 "http://localhost:${port}/q/dev-ui/" 2>/dev/null) && {
        echo "  Dev UI:     AVAILABLE"
    } || echo "  Dev UI:     not available"
}
...
# ─── QUARKUS CONFIG (from running services) ───────────────────────────────────

header "5. QUARKUS EFFECTIVE CONFIG (key properties)"

check_config() {
    local name="$1"
    local port="$2"

    # Try to read config via Quarkus dev endpoints
    if ! (echo >/dev/tcp/localhost/"$port") 2>/dev/null; then
        return
    fi
    ...
    local props=(
        ...
        "pipestream.registration.advertised-port"
        "pipestream.registration.internal-host"
        "pipestream.registration.internal-port"
        "pipestream.registration.mode"
        "pipestream.registration.required"
        "pipestream.registration.http.enabled"
        "pipestream.registration.http.health-path"
        "pipestream.registration.http.health-url"
        "pipestream.server.host-mode"
    )

    for prop in "${props[@]}"; do
        local val
        val=$(curl -sf --max-time 2 "http://localhost:${port}/q/config/${prop}" 2>/dev/null) || val="<unavailable>"
        printf "  %-50s = %s\n" "$prop" "$val"
    done
}

# Only check a few key services to keep it fast
for entry in "platform-registration:38101" "parser:39001" "opensearch-sink:39004"; do
    IFS=: read -r name port <<< "$entry"
    check_config "$name" "$port"
done
...
subheader "Gradle wrapper versions"
for dir in /work/core-services/platform-registration-service /work/modules/module-parser /work/modules/module-opensearch-sink; do
    if [[ -f "$dir/gradle/wrapper/gradle-wrapper.properties" ]]; then
        local_ver=$(grep distributionUrl "$dir/gradle/wrapper/gradle-wrapper.properties" 2>/dev/null | grep -oP 'gradle-\K[0-9.]+' || echo "unknown")
        echo "  $(basename "$dir"): Gradle $local_ver"
    fi
done
